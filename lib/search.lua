...
function webview.methods.start_search(view, w, text)
    if string.match(text, "^[?/]") then
        w:set_mode("search")
...
function webview.methods.search(view, w, text, forward, wrap)
...
function webview.methods.clear_search(view, w, clear_state)
    w.ibar.input.fg = theme.ibar_fg
    w.ibar.input.bg = theme.ibar_bg
    view:clear_search()
